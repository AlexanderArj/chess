...
async function initApp() {
  ...
  async function play() {
    for (let i = 0; i < movesToPlay.length; i++) {
      ...
      const pieceToMove = piecesData.find(
        p => p.file === squareR.file && p.rank === squareR.rank
      );

      let enemyPiece = piecesData.find(
        ep =>
          ep.file === squareD.file &&
          ep.rank === squareD.rank &&
          ep.color != pieceToMove.color
      );
